...
function decodedTextCandidates(text) {
  const candidates = [];
  // Examine every start offset with a fixed encoded-assignment prefix prefilter.
  ...
  const encodedAssignmentPrefixes = new Set(
    ['api_key=', 'api-key=', 'apikey=', 'secret=', 'access_token=', 'access-token=', 'password=']
      .flatMap((key) => ['base64', 'base64url'].map((encoding) => Buffer.from(key.slice(0, 3)).toString(encoding).slice(0, 4))),
  );
