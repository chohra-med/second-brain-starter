...
function decodedTextCandidates(text) {
  ...
  for (const token of text.split(/[ \t\r\n]+/).filter(Boolean)) {
    ...
    for (const match of runs) {
      const run = match[0];
      if (run.length > 16) {
        const boundary = [...run].findIndex((character) => /[+/_-]/.test(character));
        if (boundary < 0) continue;
        const before = run.slice(0, boundary);
        const after = run.slice(boundary + 1);
        if (before.length > 0 && before.length <= 16) positions.push([before]);
        if (after.length > 0 && after.length <= 16) positions.push([after]);
        continue;
      }
      ...
      positions.push([...alternatives].filter(Boolean));
    }
  }
