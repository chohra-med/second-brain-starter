...
function decodedTextCandidates(text) {
  ...
  // Keep wrapped candidates bounded to whitespace-delimited chunks. A greedy
  // regex can swallow ordinary prose on either side and miss the real run.
  ...
  for (let start = 0; start < chunks.length; start += 1) {
    ...
    for (let end = start; end < chunks.length && end < start + 128; end += 1) {
      ...
      compact += chunk;
      if (compact.length >= 16) base64Tokens.set(compact, true);
    }
  }
