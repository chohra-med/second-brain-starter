...
function decodedTextCandidates(text) {
  ...
  const base64Tokens = new Set(text.match(/[A-Za-z0-9+/_-]{16,}={0,2}/g) ?? []);
  let wrapped = [];
  const flushWrapped = () => {
    if (wrapped.length > 1) base64Tokens.add(wrapped.join(''));
    wrapped = [];
  };
  for (const line of text.split(/\r?\n/)) {
    const token = line.trim();
    if (/^[A-Za-z0-9+/_-]{4,}={0,2}$/.test(token)) wrapped.push(token);
    else flushWrapped();
  }
  flushWrapped();
  for (const token of base64Tokens) {
    ...
    try {
      ...
      if (decoded && !decoded.includes('\uFFFD')) candidates.push(decoded);
    } catch {
