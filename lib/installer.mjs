...
const CREDENTIAL_ASSIGNMENT = /\b(?:api[-_]?key|secret|access[-_]?token|password)\s*[:=]\s*["']?[A-Za-z0-9_./+\-=]{8,}/i;
...
async function assertTransactionNamespace(target) {
  for (const [relative, label] of [
    [STATE_RELATIVE_PATH, 'Installed state namespace'],
    [RECEIPTS_RELATIVE_DIRECTORY, 'Receipt namespace'],
    [BACKUPS_RELATIVE_DIRECTORY, 'Backup namespace'],
  ]) {
    await assertNoSymlinkAncestors(path.join(target, ...relative.split('/')), label, target);
  }
}
...
function containsSecretBearingContent(bytes) {
  ...
  return /-----BEGIN(?: [A-Z]+)? PRIVATE KEY-----/i.test(text)
    || /\bauthorization\s*:\s*(?:bearer|basic|token)\s+\S+/i.test(text)
