...
function normalizeState(state) {
  ...
  if (state.schemaVersion !== 1) fail('INVALID_STATE', 'Installed state schemaVersion must be 1.');
  ...
  if (!SHA256_PATTERN.test(state.manifestSha256 ?? '') || !SHA256_PATTERN.test(state.planDigest ?? '')) {
    fail('INVALID_STATE', 'Installed state manifestSha256 and planDigest must be lowercase SHA-256 values.');
  }
  if (typeof state.transactionId !== 'string' || !/^tx-[0-9a-f-]+$/.test(state.transactionId)) {
    fail('INVALID_STATE', 'Installed state transactionId must be a transaction receipt ID.');
  }
  if (state.operation !== undefined && !['init', 'upgrade'].includes(state.operation)) {
    fail('INVALID_STATE', 'Installed state operation must be init or upgrade when present.');
  }
  ...
  for (const [destination, record] of Object.entries(state.managedPaths)) {
    ...
    if (record.mergeKind === 'managed-block' && !SHA256_PATTERN.test(record.managedBlockSha256 ?? '')) {
...
async function assertReceiptRollbackContract(target, receipt, receiptIdValue) {
  ...
  for (const write of receipt.writes) {
    if (write.destination === STATE_RELATIVE_PATH) continue;
